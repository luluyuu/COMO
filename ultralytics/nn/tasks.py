...
class BaseModel(torch.nn.Module):
    ...
    def loss(self, batch, preds=None):
        ...
        # 这里修改了，将batch进行拼接
        combined_batch = torch.cat([batch["img_rgb"], batch["img_ir"]], dim=1)
        preds = self.forward(combined_batch) if preds is None else preds
...
class DetectionModel(BaseModel):
    ...
    def _predict_once(self, x_in, profile=False,  visualize=False, embed=None):
        """
        ...
        if x_in.size(1) != 3 :  # batch size
            x = x_in[:, 0:3, :, :]  # select RGB channel
            x_ir = x_in[:, 3:6, :, :]  # select IR channel
            x3 = x_in[:, 0:3, :, :]
            for m in self.model:
                # print("Moudle", i)
                if m.f != -1:  # if not from previous layer
...
class RTDETRDetectionModel(DetectionModel):
    ...
    def loss(self, batch, preds=None):
        ...
        combined_img = torch.cat([img_rgb, img_ir], dim=1)
        preds = self.predict(combined_img, batch=targets) if preds is None else preds
    ...
    def predict(self, x_in, profile=False, visualize=False, batch=None, augment=False, embed=None):
        """
