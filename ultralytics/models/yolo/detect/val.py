...
class DetectionValidator(BaseValidator):
        ...
        for si, pred in enumerate(preds):
            ...
            if self.args.save_txt:
                self.save_one_txt(
                    ...
                    self.args.save_conf,
                    pbatch["ori_shape"],
                    self.save_dir / "labels" / f"{Path(batch['im_file_rgb'][si]).stem}.txt",
                )
    ...
    def build_dataset(self, img_path, mode="val", batch=None, multi=False):
        """
        Build YOLO Dataset.
        ...
        """
        return build_yolo_dataset(self.args, img_path, batch, self.data, multi=multi, mode=mode, stride=self.stride)

    def get_dataloader(self, dataset_path, batch_size, multi=False):
        """
        ...
        """
        dataset = self.build_dataset(dataset_path, batch=batch_size, mode="val", multi=multi)
