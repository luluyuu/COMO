...
class BaseDataset(Dataset):
    ...
    def __init__(
        ...
        self.im_files_rgb, self.im_files_ir = self.get_img_files(self.img_path)
        ...
        self.ims_rgb, self.ims_ir, self.im_hw0, self.im_hw = [None] * self.ni, [None] * self.ni, [None] * self.ni, [None] * self.ni
        self.npy_files = [Path(f).with_suffix(".npy") for f in self.im_files_rgb]
    ...
    def get_img_files(self, img_path):
        ...
        try:
            ...
            for p in img_path if isinstance(img_path, list) else [img_path]:
                ...
                if p.is_dir():  # dir
                    f_rgb += glob.glob(str(p / "*.*"), recursive=True)
                    f_ir = [f.replace(m1, m2) for f in f_rgb]
                    # F = list(p.rglob('*.*'))  # pathlib
                elif p.is_file():  # file
                    # 第一版本多模态
                    # with open(p, encoding="utf-8") as t:
                    #     t = t.read().strip().splitlines()
                    #     t = [x+'.jpg' for x in t]  # add .jpg extension to each line
                    #     parent = str(p.parent) + os.sep
                    #     # f += [x.replace("./", parent) if x.startswith("./") else x for x in t]  # local to global path
                    #     parent_rgb = str(p.parent) + "RGB" + os.sep if parent.endswith(os.sep) else str(p.parent) + os.sep + "RGB" + os.sep
                    #     parent_ir = str(p.parent) + "TIR" + os.sep if parent.endswith(os.sep) else str(p.parent) + os.sep + "IR" + os.sep
                    #     f_rgb += [str(parent_rgb) + x for x in t ]
                    #     f_ir += [str(parent_ir) + x for x in t ]  # local to global path
                    ...
                    with open(p, encoding="utf-8") as file:
                        # 读取并处理基础文件名
                        base_names = [x.strip() + '.jpg' for x in file.read().splitlines()]  # 强制添加.jpg扩展名
                        
                        # 构建目录路径
                        base_dir = Path(p).parent
                        rgb_dir = base_dir / m1
                        ir_dir = base_dir / m2  # 根据实际目录名调整
                        
                        # 生成完整路径列表
                        batch_rgb = [str(rgb_dir / name) for name in base_names]
                        batch_ir = [str(ir_dir / name) for name in base_names]
                        ...
                        rgb_dict = {Path(p).stem.lower(): p for p in batch_rgb}
                        ir_dict = {Path(p).stem.lower(): p for p in batch_ir}
                    ...
                    # 最终错误报告
                    if missing_pairs:
                        print(f"\033[1;32m验证完成: 有效数据对 {len(f_rgb)}/{len(base_names)} | "
                            f"丢弃 {len(missing_pairs)} 个无效项\033[0m")
                        # for idx, (rgb, ir, reason) in enumerate(missing_pairs, 1):
                        #     print(f"  {idx}. {Path(rgb).name}")
                        #     print(f"     \033[33mMissing -> {reason}\033[0m")
                        #     print(f"     RGB路径: {rgb}")
            ...
            im_files_rgb = sorted(x.replace("/", os.sep) for x in f_rgb if x.split(".")[-1].lower() in IMG_FORMATS)
            im_files_ir = sorted(x.replace("/", os.sep) for x in f_ir if x.split(".")[-1].lower() in IMG_FORMATS)
            # self.img_files = sorted([x for x in f if x.suffix[1:].lower() in IMG_FORMATS])  # pathlib
            assert im_files_rgb, f"{self.prefix}No RGB images found in {img_path}. {FORMATS_HELP_MSG}"
            assert im_files_ir, f"{self.prefix}No IR images found in {img_path}. {FORMATS_HELP_MSG}"
        ...
        check_file_speeds(im_files_rgb, prefix=self.prefix)  # check image read speeds
        check_file_speeds(im_files_ir, prefix=self.prefix)  # check image read speeds
        return im_files_rgb, im_files_ir
    ...
    def load_image(self, i, rect_mode=True):
        ...
        """
        ims_rgb, ims_ir, f_rgb, f_ir, fn = self.ims_rgb[i], self.ims_ir[i], self.im_files_rgb[i], self.im_files_ir[i], self.npy_files[i]
        # cache也是false,还没开启
        if ims_rgb is None:  # not cached in RAM
            if fn.exists():  # load npy
                try:
                    ...
                    im_rgb = imread(f_rgb, flags=self.cv2_flag)  # BGR
                    im_ir = imread(f_ir, flags=self.cv2_flag)  # BGR
            else:  # read image
                im_rgb = imread(f_rgb, flags=self.cv2_flag)  # BGR
                im_ir = imread(f_ir, flags=self.cv2_flag)  # BGR
            if im_rgb is None and im_ir is None:
                raise FileNotFoundError(f"Image Not Found {f_rgb} or {f_ir} in {self.img_path}")
            ...
            # augment 还没改
            if self.augment:
                self.ims_rgb[i], self.ims_ir[i], self.im_hw0[i], self.im_hw[i] = im_rgb, im_ir, (h0, w0), im_rgb.shape[:2]  # im, hw_original, hw_resized
                self.buffer.append(i)
                if 1 < len(self.buffer) >= self.max_buffer_length:  # prevent empty buffer
                    j = self.buffer.pop(0)
                    if self.cache != "ram":
                        self.ims_rgb[j], self.ims_ir[j], self.im_hw0[j], self.im_hw[j] = None, None, None, None

            return im_rgb, im_ir, (h0, w0), im_rgb.shape[:2]

        return self.ims_rgb[i], self.ims_ir[i], self.im_hw0[i], self.im_hw[i]
    ...
    def get_image_and_label(self, index):
        ...
        label["img_rgb"], label["img_ir"], label["ori_shape"], label["resized_shape"] = self.load_image(index)
        label["ratio_pad"] = (
