...
def build_yolo_dataset(cfg, img_path, batch, data, multi, mode="train", rect=False, stride=32, multi_modal=False):
...
def check_source(source):
    ...
    """
    webcam, screenshot, from_img, in_memory, tensor, multi_modal = False, False, False, False, False, False
    if isinstance(source, (str, int, Path)):  # int for local usb camera
        ...
        # 多模态数据判断（列表中全部为路径）
        if all(isinstance(item, (str, Path)) for item in source) and len(source) <= 2:
            multi_modal = True
            source = [str(Path(item)) for item in source]  # 统一转换为字符串路径
        else:
            # 常规图像数组处理
    ...
    return source, webcam, screenshot, from_img, in_memory, tensor, multi_modal
...
def load_inference_source(source=None, batch=1, vid_stride=1, buffer=False):
    ...
    """
    source, stream, screenshot, from_img, in_memory, tensor, multi_modal = check_source(source)
    source_type = source.source_type if in_memory else SourceTypes(stream, screenshot, from_img, tensor, multi_modal)
    ...
    if tensor:
        ...
        dataset = LoadMultimodalImagesAndVideos(source, batch=batch, vid_stride=vid_stride)
    else:
