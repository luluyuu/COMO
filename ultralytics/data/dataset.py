...
class YOLODataset(BaseDataset):
    ...
    def cache_labels(self, path=Path("./labels.cache")):
        ...
        with ThreadPool(NUM_THREADS) as pool:
            ...
            for im_file_rgb, im_file_ir, lb, shape, segments, keypoint, nm_f, nf_f, ne_f, nc_f, msg in pbar:
                nm += nm_f
                nf += nf_f
                ne += ne_f
                nc += nc_f
                if im_file_rgb:
                    x["labels"].append(
                        {
                            "im_file_rgb": im_file_rgb,
                            "im_file_ir": im_file_ir,
                            "shape": shape,
                            "cls": lb[:, 0:1],  # n, 1
                            "bboxes": lb[:, 1:],  # n, 4
                            "segments": segments,
    ...
    def collate_fn(batch):
        ...
        for i, k in enumerate(keys):
            value = values[i]
            if k in {"img_rgb", "img_ir", "text_feats"}:
