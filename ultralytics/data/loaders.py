...
class LoadMultimodalImagesAndVideos:
    """
    A class for loading and processing images and videos for YOLO object detection.
    ...
    def __init__(self, path, batch=1, vid_stride=1):
        ...
        if isinstance(path_rgb, str) and Path(path_rgb).suffix == ".txt":  # *.txt file with img/vid/dir on each line
            parent = Path(path).parent
            path = Path(path).read_text().splitlines()  # list of sources
        files_rgb, files_ir, videos = [], [], []
        for p_rgb in sorted(path_rgb) if isinstance(path_rgb, (list, tuple)) else [path_rgb]:
            a = str(Path(p_rgb).absolute())  # do not use .resolve() https://github.com/ultralytics/ultralytics/issues/2912
            if "*" in a:
                files_rgb.extend(sorted(glob.glob(a, recursive=True)))  # glob
            elif os.path.isdir(a):
                files_rgb.extend(sorted(glob.glob(os.path.join(a, "*.*"))))  # dir
            elif os.path.isfile(a):
                files_rgb.append(a)  # files (absolute or relative to CWD)
            elif parent and (parent / p_rgb).is_file():
                files_rgb.append(str((parent / p_rgb).absolute()))  # files (relative to *.txt file parent)
            else:
                raise FileNotFoundError(f"{p_rgb} does not exist")
        
        for p_ir in sorted(path_ir) if isinstance(path_ir, (list, tuple)) else [path_ir]:
            a = str(Path(p_ir).absolute())  # do not use .resolve() https://github.com/ultralytics/ultralytics/issues/2912
            if "*" in a:
                files_ir.extend(sorted(glob.glob(a, recursive=True)))  # glob
            elif os.path.isdir(a):
                files_ir.extend(sorted(glob.glob(os.path.join(a, "*.*"))))  # dir
            elif os.path.isfile(a):
                files_ir.append(a)  # files (absolute or relative to CWD)
            elif parent and (parent / p_ir).is_file():
                files_ir.append(str((parent / p_ir).absolute()))  # files (relative to *.txt file parent)
            else:
            ...
            ni, nv = len(files_rgb), len(videos)

            self.files_rgb = files_rgb
            self.files_ir = files_ir
            self.nf = ni + nv  # number of files
            self.ni = ni  # number of images
            self.video_flag = [False] * ni + [True] * nv
            self.mode = "video" if ni == 0 else "image"  # default to video if no images
    ...
    def __next__(self):
        """Returns the next batch of images or video frames with their paths and metadata."""
        paths_rgb, paths_ir, imgs_rgb, imgs_ir, info = [], [], [], [], []
        while len(imgs_rgb) < self.bs:
            if self.count >= self.nf:  # end of file list
                if imgs_rgb:
                    return paths_rgb, path_ir, imgs_rgb, imgs_ir, info  # return last partial batch
                else:
                    raise StopIteration
            ...
            if self.video_flag[self.count]:
                ...
                # Handle image files (including HEIC)
                self.mode = "image"
                if path_rgb.split(".")[-1].lower() == "heic":
                    # Load HEIC image using Pillow with pillow-heif
                    check_requirements("pillow-heif")
                    ...
                    im0_rgb = imread(path_rgb)  # BGR
                    im0_ir = imread(path_ir)  # BGR
                if im0_rgb is None:
                    LOGGER.warning(f"Image Read Error {path_rgb}")
                elif im0_ir is None:
                    LOGGER.warning(f"Image Read Error {path_ir}")
                else:
                    paths_rgb.append(path_rgb)
                    paths_ir.append(path_ir)
                    imgs_rgb.append(im0_rgb)
                    imgs_ir.append(im0_ir)
                    info.append(f"RGB image {self.count + 1}/{self.nf} {path_rgb}: ")
                    info.append(f"IR image {self.count + 1}/{self.nf} {path_ir}: ")
                self.count += 1  # move to the next file
                if self.count >= self.ni:  # end of image list
                    break

        return paths_rgb, paths_ir, imgs_rgb, imgs_ir, info
