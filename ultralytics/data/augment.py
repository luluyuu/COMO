...
class RandomPerspective:
    ...
    def affine_transform(self, img0, img1, border):
        """
        ...
        C[0, 2] = -img0.shape[1] / 2  # x translation (pixels)
        C[1, 2] = -img0.shape[0] / 2  # y translation (pixels)
        ...
        if (border[0] != 0) or (border[1] != 0) or (M != np.eye(3)).any():  # image changed
            if self.perspective:
                img0 = cv2.warpPerspective(img0, M, dsize=self.size, borderValue=(114, 114, 114))
                img1 = cv2.warpPerspective(img1, M, dsize=self.size, borderValue=(114, 114, 114))
            else:  # affine
                img0 = cv2.warpAffine(img0, M[:2], dsize=self.size, borderValue=(114, 114, 114))
                img1 = cv2.warpAffine(img1, M[:2], dsize=self.size, borderValue=(114, 114, 114))
        return img0, img1, M, s
...
class RandomHSV:
    ...
    def __call__(self, labels):
        ...
        img_rgb = labels["img_rgb"]
        img_ir = labels["img_ir"]
        if img_rgb.shape[-1] != 3:  # only apply to RGB images
            return labels
        if self.hgain or self.sgain or self.vgain:
            ...
            if img_ir is not None:
                hue, sat, val = cv2.split(cv2.cvtColor(img_ir, cv2.COLOR_BGR2HSV))
                im_hsv = cv2.merge((cv2.LUT(hue, lut_hue), cv2.LUT(sat, lut_sat), cv2.LUT(val, lut_val)))
                cv2.cvtColor(im_hsv, cv2.COLOR_HSV2BGR, dst=img_ir)
        return labels
...
class RandomFlip:
    ...
    def __call__(self, labels):
        ...
        """
        img_rgb, img_ir = labels["img_rgb"], labels["img_ir"]
        instances = labels.pop("instances")
...
class LetterBox:
    ...
    def __call__(self, labels=None, image=None):
        ...
        img_ir = labels.get("img_ir") if image is None else image
        ...
        if c == 3:
            ...
            pad_img_rgb = np.full((h + top + bottom, w + left + right, c), fill_value=114, dtype=img_rgb.dtype)
            pad_img_rgb[top : top + h, left : left + w] = img_rgb
            img_rgb = pad_img_rgb

            pad_img_ir = np.full((h + top + bottom, w + left + right, c), fill_value=114, dtype=img_ir.dtype)
            pad_img_ir[top : top + h, left : left + w] = img_ir
            img_ir = pad_img_ir
...
class CopyPaste(BaseMixTransform):
    ...
    def _transform(self, labels1, labels2={}):
        """Applies Copy-Paste augmentation to combine objects from another image into the current image."""
        im_rgb = labels1["img_rgb"]
        im_ir = labels1["img_ir"]
        cls = labels1["cls"]
        h, w = im_rgb.shape[:2]
        ...
        im_new_rgb = np.zeros(im_rgb.shape, np.uint8)
        ...
        for j in indexes[: round(self.p * n)]:
            ...
            cv2.drawContours(im_new_rgb, instances2.segments[[j]].astype(np.int32), -1, (1, 1, 1), cv2.FILLED)
            cv2.drawContours(im_new_ir, instances2.segments[[j]].astype(np.int32), -1, (1, 1, 1), cv2.FILLED)

        result_rgb = labels2.get("img_rgb", cv2.flip(im_rgb, 1))  # augment segments
        result_ir = labels2.get("img_ir", cv2.flip(im_ir, 1))  # augment segments
        i = im_new_rgb.astype(bool)
        im_rgb[i] = result_rgb[i]
        im_ir[i] = result_ir[i]

        labels1["img_rgb"] = im_rgb
        labels1["img_ir"] = im_ir
        labels1["cls"] = cls
...
class Albumentations:
    ...
    def __call__(self, labels):
        ...
        im_ir = labels["img_ir"]
        if im_rgb.shape[2] != 3:  # Only apply Albumentation on 3-channel images
            return labels
        ...
        if self.contains_spatial:
            ...
            if len(cls):
                ...
                new_ir = self.transform(image=im_ir, bboxes=bboxes, class_labels=cls)  # transformed
                if len(new_rgb["class_labels"]) > 0:  # skip update if no bbox in new im
                    labels["img_rgb"] = new_rgb["image"]
                    labels["img_ir"] = new_ir["image"]
                    labels["cls"] = np.array(new_rgb["class_labels"])
                    bboxes = np.array(new_rgb["bboxes"], dtype=np.float32)
                labels["instances"].update(bboxes=bboxes)
        else:
            labels["img_rgb"] = self.transform(image=labels["img_rgb"])["image"]  # transformed
            labels["img_ir"] = self.transform(image=labels["img_ir"])["image"]
