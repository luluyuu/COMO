...
def img2label_paths(img_paths):
    ...
    for x in img_paths:
        ...
        # VEDAI
        # t.append(x.replace(sa, sb, 1).replace(x[-7:], '.txt'))
    return t
...
def verify_image_label(args):
    ...
    im_file_rgb, im_file_ir, lb_file, prefix, keypoint, num_cls, nkpt, ndim, single_cls = args
    # Number (missing, found, empty, corrupt), message, segments, keypoints
    nm, nf, ne, nc, msg, segments, keypoints = 0, 0, 0, 0, "", [], None
    try:
        # Verify images
        im_rgb = Image.open(im_file_rgb)
        im_rgb.verify()  # PIL verify
        shape = exif_size(im_rgb)  # image size
        shape = (shape[1], shape[0])  # hw
        assert (shape[0] > 9) & (shape[1] > 9), f"image size {shape} <10 pixels"
        assert im_rgb.format.lower() in IMG_FORMATS, f"invalid image format {im_rgb.format}. {FORMATS_HELP_MSG}"
        if im_rgb.format.lower() in {"jpg", "jpeg"}:
            with open(im_file_rgb, "rb") as f:
                f.seek(-2, 2)
                if f.read() != b"\xff\xd9":  # corrupt JPEG
                    ImageOps.exif_transpose(Image.open(im_file_rgb)).save(im_file_rgb, "JPEG", subsampling=0, quality=100)
                    msg = f"{prefix}{im_file_rgb}: corrupt JPEG restored and saved"

        im_ir = Image.open(im_file_ir)
        im_ir.verify()
        shape = exif_size(im_ir)  # image size
        shape = (shape[1], shape[0])  # hw
        assert (shape[0] > 9) & (shape[1] > 9), f"image size {shape} <10 pixels"
        assert im_ir.format.lower() in IMG_FORMATS, f"invalid image format {im_ir.format}. {FORMATS_HELP_MSG}"
        if im_ir.format.lower() in {"jpg", "jpeg"}:
            with open(im_file_ir, "rb") as f:
                f.seek(-2, 2)
                if f.read() != b"\xff\xd9":  # corrupt JPEG
                    ImageOps.exif_transpose(Image.open(im_file_ir)).save(im_file_ir, "JPEG", subsampling=0, quality=100)
                    msg = f"{prefix}{im_file_ir}: corrupt JPEG restored and saved"
        ...
        if os.path.isfile(lb_file):
            ...
            if nl := len(lb):
                ...
                if len(i) < nl:  # duplicate row check
                    ...
                    msg = f"{prefix}{im_file_rgb}: {nl - len(i)} duplicate labels removed"
            else:
                ne = 1  # label empty
        ...
        lb = lb[:, :5]
        return im_file_rgb, im_file_ir, lb, shape, segments, keypoints, nm, nf, ne, nc, msg
    except Exception as e:
        nc = 1
        msg = f"{prefix}{im_file_rgb}{im_file_ir}: ignoring corrupt image/label: {e}"
