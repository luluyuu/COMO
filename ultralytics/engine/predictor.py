...
class BasePredictor:
    ...
    def preprocess(self, im0, im1):
        ...
        """
        not_tensor1 = not isinstance(im0, torch.Tensor)
        not_tensor2 = not isinstance(im1, torch.Tensor)
        if not_tensor1:
            im0 = np.stack(self.pre_transform(im0))
            if im0.shape[-1] == 3:
                im0 = im0[..., ::-1]
            im0 = im0.transpose((0, 3, 1, 2))
            im0 = np.ascontiguousarray(im0)  # contiguous
            im0 = torch.from_numpy(im0)

        if not_tensor2:
            im1 = np.stack(self.pre_transform(im1))
            if im1.shape[-1] == 3:
                im1 = im1[..., ::-1]
            im1 = im1.transpose((0, 3, 1, 2))  # BHWC to BCHW, (n, 3, h, w)
            im1 = np.ascontiguousarray(im1)  # contiguous
            im1 = torch.from_numpy(im1)
            
        im = torch.cat((im0, im1), dim=1)  # (n, 6, h, w)
    ...
    def postprocess(self, preds, img_rgb, img_ir, orig_imgs_rgb, orig_imgs_ir):
    ...
    def stream_inference(self, source=None, model=None, *args, **kwargs):
        ...
        with self._lock:  # for thread-safe inference
            ...
            for self.batch in self.dataset:
                ...
                if self.dataset.source_type.multi_modal:
                    paths_rgb, paths_ir, im0s_rgb, im0s_ir, s = self.batch
                else :
                    paths, im0s, s = self.batch
                    paths_rgb, paths_ir = paths, paths
                    im0s_rgb, im0s_ir = im0s, im0s

                # Preprocess
                with profilers[0]:
                    im = self.preprocess(im0s_rgb, im0s_ir)
                ...
                # Postprocess
                with profilers[2]:
                    self.results = self.postprocess(preds, im[:,0:3,:,:], im[3:6], im0s_rgb, im0s_ir)
                ...
                for i in range(n):
                    ...
                    if self.args.verbose or self.args.save or self.args.save_txt or self.args.show:
                        s[i] += self.write_results(i, Path(paths_rgb[i]), im, s)
